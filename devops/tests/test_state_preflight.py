...
class StatePreflightTests(unittest.TestCase):
    ...
    def test_wrong_account_stops_before_any_state_read(self):
        argv = ['state_preflight.py', '--bucket', 'test', '--key', 'dev/vpc/terraform.tfstate', '--region', 'ap-northeast-2', '--account', '123456789012']
        with patch('sys.argv', argv), patch('state_preflight.aws_json', return_value={'Account': '999999999999'}) as aws:
